...
import {
  ...
  ROWS_PER_PAGE,
  STATUS_MAP_REV,
  type ConfirmedBookingsReportByBranch,
...
const ConfirmedBookings = () => {
  const [formData, setFormData] =
    useState<ConfirmedBookingsReportByBranch | null>({
  ...
  const [
    filteredConfirmedBookingsForAHotel,
    setFilteredConfirmedBookingsForAHotel,
    ...
    function loadAllBookingForABranch() {
      ...
      setFilteredConfirmedBookingsForAHotel(
        bookingsForAHotelData
          .filter((b) => b.tk_bookingstatus === STATUS_MAP_REV["Confirmed"])
          .sort((a, b) => {
  ...
  function handleSelectedChange(e: SelectChangeEvent) {
    ...
    setFormData((prevData: ConfirmedBookingsReportByBranch | null) => {
      if (!prevData) {
    ...
    <>
      <div className="flex flex-col w-full ">
        ...
        <div className="flex flex-col w-[80%] m-auto bg-white my-3">
          <div className="flex flex-col justify-around py-2">
            <div className="flex w-full justify-around my-4">
              <ThemeProvider theme={formHeaderTitleTypographyTheme}>
                <Typography color="black">Confirmed Bookings</Typography>
              </ThemeProvider>
            </div>
          ...
          {filteredConfirmedBookingsForAHotel.length === 0 && (
              ...
              <Card className="flex m-auto my-4">
                <CardContent>
                  <Typography sx={{ color: "text.secondary", fontSize: 20 }}>
                    {`Sorry No confirmed bookings for ${branchNameLookup(branches)[formData?._tk_hotelbranch_value || ""]} `}
                  </Typography>
                </CardContent>
              </Card>
            </ThemeProvider>
      ...
      {showReport && filteredConfirmedBookingsForAHotel.length > 0 && (
        <div
          ...
          <div className="text-center mt-3 bg-gray-400 mx-2">
            <ThemeProvider theme={formHeaderTitleTypographyTheme}>
              <Typography color="black">
                {`Confimed Bookings for ${branchNameLookup(branches)[formData?._tk_hotelbranch_value || ""]}`}
              </Typography>
            </ThemeProvider>
          </div>

          <div className="flex flex-wrap flex:0 0 50%">
            {filteredConfirmedBookingsForAHotel.map((booking) => {
              return (
                <Card
